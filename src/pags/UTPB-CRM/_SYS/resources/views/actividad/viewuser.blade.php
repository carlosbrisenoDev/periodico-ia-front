    ...
    <div class="card card-default">
        <div class="card-body">
          <div class="card-header">
            <h3>Actividades del Usuario {{$user->full_name()}}</h3>
            <div class="row">
            ...
            <div id="tablesContent">
              @if (count($acts) > 0)
                ...
                <table class="table table-stripped">
                  ...
                  <tbody>
                    <tr>
                      <td>{{$user->actividadesRealizadas()}}</td>
                      <td>
                        @php
                          $min = $user->tiempoActividades();
                          $hour = $min;
...
<script>
  ...
  $(document).on('click','.btn-find-dates', async function(e) {
    ...
    dateQuery = await serverResponse({_token:'{{csrf_token()}}',rangeDate:dr.value,user:user},'/api/actividades/getByDate/byuser');
    dateGQuery = await serverResponse({_token:'{{csrf_token()}}',rangeDate:dr.value,user:user},'/api/actividades/getByDate/general/byuser');
    $('#tablesContent').html('');
