...
<html lang="en" dir="ltr">
  <head>
    ...
    <meta name="agent" content="1">
    <meta name="from" content="David Valdivia">
    ...
    <link rel="stylesheet" href="/css/master.css?r=1">
    <link rel="stylesheet" href="/css/tooltip.css?r=1">
  </head>
  <body>
  ...
  <script>
      ...
      var audio = new Audio('/sounds/unpop.mp3');
      ...
      var socket = io.connect('', {query: 'roomid='+roomid+"&agent="+agent.id});
      $(() => {
          $("#send").click(()=>{
              sender();
          });
            ...
            <div class="clearfix">
              <div class="${message.from == "user" ? "float-left" : "float-right fromhere"}">
                <div class="chat-bubble chat-bubble--${message.from == "user" ? "left" : "right"}">
                  ...
                  <span>
                    ${message.mime && message.type == "document" ? `<div class='document-chat'><a download href='/resources/${message.image}.${message.mime.split("/")[1]}'><i class="fa fa-file fa-3x"></i></a></div></br>` : ""}
                    ${message.mime && message.type == "image" ? `<img onclick="openLink(this)" class='image-chat' onerror="loadError(this)" src='/resources/${message.image}.${message.mime.split("/")[1]}' /></br>` : ""}
                    ${message.mime && message.type == "sticker" ? `<img class='stiker-chat' onerror="loadError(this)" src='/resources/${message.image}.${message.mime.split("/")[1]}' /></br>` : ""}
                    ${message.mime && message.type == "audio" ? `<audio class="audio-chat" controls><source src="/resources/${message.image}.${message.mime.split("/")[1]}" type="${message.mime}"></audio></br>` : ""}
                    ${message.mime && message.type == "video" ? `<video class="video-chat" controls><source src="/resources/${message.image}.${message.mime.split("/")[1]}" type="${message.mime}"></video></br>` : ""}
      ...
      function getChats(){
        $.get(`/chats/unreads/${agent.id}`, (data) => {
          console.log(data);
      ...
      function sendPlantilla(){
        ...
        Swal.fire({
          ...
          preConfirm : val => {
            $.post('/sendplantilla',{
              roomid, 
              message:`Se ha enviado una plantilla ${val}`,
              plantilla : val,
              text1 : inputVars[val],
              name:agent.name,
              from:agent.name,
