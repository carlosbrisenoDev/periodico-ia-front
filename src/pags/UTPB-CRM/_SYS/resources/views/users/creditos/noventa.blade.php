    ...
    <div class="col-12">
      <div class="card">
        <div class="card-body">
          ...
          <h6 class="card-subtitle mb-2 text-muted">Notificar pagos pendientes 60 días</h6>
          <hr>
            <div class="row">
                @php
                  $i = 1;
                ...
                @foreach (\App\pagos::where("pagado",NULL)->orderBy("created_at","desc")->get() as $pago)
                  @if ((\Carbon\carbon::parse($pago->anio."-".$pago->mes_en."-1")->subDays(1)->isPast() && $pago->tabla->cliente->baja == NULL) && $pago->status != 9)
                    @php
                      $atraso = \Carbon\carbon::parse($pago->anio."-".$pago->mes_en."-1")->diffInDays(\Carbon\carbon::now());
                      $bg = $atraso >= 60 ? "bg-little" : ($atraso >= 30 ? "bg-warning" : "");
                    @endphp
                    @if ($atraso >= 60)
