...
class creditos extends Controller
{
  public function seto(Request $r){
    $c = \App\credito_info::whereRAW("md5(id)='$r->cid'")->first();
    eval("\$c->".$r->seto."='".$r->v."';");
    $c->save();
    return "Ok";
  }
  ...
  public function carteraadd(Request $r){
    $c = \App\cliente::whereRAW("md5(id)='$r->cid'")->first();
    $c->credito_info->status = "cartera";
    $c->credito_info->save();
    return redirect("/creditos/creditos?cid=".md5($c->id))->with("status","Cliente agregado");
  }
  public function requestsign(Request $r){
    $c = \App\cliente::whereRAW("md5(id)='$r->cid'")->first();
    $c->cocredito->status = 2;
    $c->credito_info->status = "firmando";
    if($c->agente != null){
    ...
    $c->cocredito->save();
    $c->credito_info->save();
    return redirect("/creditos/solicitud?cid=".md5($c->credito_info->id))->with("status","Firma solicitada");
  }
  public function actualizar(Request $r){
    ...
    $c->status = "enviado";
    //dd($r->nombre);
    if(is_array($r->nombre) && count($r->nombre) > 0){
